...
export async function handleAddIntent(ctx: LineBotContext, parsed: ParsedMessage): Promise<any> {
  if (parsed.intent === 'add_pet' && parsed.petName) {
    ...
    const text = MessageUI.confirmAddPet(parsed.petName, parsed.petType || '未知');
    return ctx.sendConfirm(text, [
  ...
  if (parsed.intent === 'add_diary' && parsed.petName && parsed.action) {
    const text = MessageUI.confirmAddDiary(parsed.action, parsed.description || '', parsed.time || '', parsed.petName);
    ...
    return ctx.sendButtons(text, '確認日記內容', [
